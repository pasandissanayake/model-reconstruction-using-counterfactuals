...
#Added by SD
def gaussian_volume(model, x, y, K=1000, sigma=0.1, num_class=2):
    output_scores = []
    baseline = tf.random.normal([K, x.shape[1]], mean=0, stddev=sigma, dtype=x.dtype)
    x_len = x.shape[0]
    for i in range(x_len):
        x_in = tf.repeat(tf.expand_dims(x[i],0),K,axis=0) + baseline
        per_output = model(x_in, training=False)[:,1] 
        output_of_interest = tf.reduce_mean(per_output, axis=0)-tf.math.reduce_std(per_output, axis=0)
        output_scores.append(tf.expand_dims(output_of_interest,0))        
...
#Added by SD
def gaussian_volume2(model, x, y, K=1000, sigma=0.1, num_class=2):
    output_scores = []
    baseline = tf.random.normal([K, x.shape[1]], mean=0, stddev=sigma, dtype=x.dtype)
    x_len = x.shape[0]
    for i in range(x_len):
...
def batch_robx_search(model,
    ...
    for step in range(max_steps):
        with tf.GradientTape() as tape:
            tape.watch(delta)
            output_scores = gaussian_volume2(model,
                                            delta,
                                            y,
                                            K=K,
                                            sigma=sigma,
...
#Added by SD
def RobX(model, X, Y, batch_size=256,  **kwargs):
    pb = tf.keras.utils.Progbar(target=X.shape[0])
    optimal_adv, y_pred, best_diff = [], [], []
    for i in range(0, X.shape[0], batch_size):
        x = X[i:i + batch_size]
        y = Y[i:i + batch_size]
        opt, y_p, diff = batch_robx_search(model, x, y, **kwargs)
        optimal_adv.append(opt)
        y_pred.append(y_p)
        best_diff.append(diff)
